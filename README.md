...
<div style= "text-align:left;">
    ...
    <h4>고객 페이지</h4>
    <div>회원가입 시 아이디 중복체크 버튼 클릭시 팝업창으로 DB 호출하여 중복 체크를 하도록 구현하였고, 패스워드는 정규식을 통해 정규식을 만족해야만 회원가입이 완료되게 구현하였고. 주소 찾기 버튼 클릭시 다음 API를 이용한 주소검색 팝업창을 통해 주소를 입력받게 구현하였습니다. 제품 페이지에서는 select문을 통해 제품리스트를 선택할 수 있게 구현하였고, 제품 이미지와 수량, 제품 총액이 실시간으로 변경되게 구현하였습니다. 장바구니에 담는 버튼 클릭시 해당 제품이 장바구니에 담기게해서 여러 제품을 한번에 구매 가능하게 구현하였고, 주문 완료시 제품 재고가 감소되게 구현하였습니다. 주문 내역 페이지에서는 총 주문내역이 모두 나오게 구현하였고, 프로필 수정페이지도 구현하여서 개인정보를 수정할 수 있게 구현하였습니다. 로그아웃 클릭시에는 세션이 끊기고 메인페이지로 이동되게 구현하였습니다.</div>
    </div>
    <br>
    <div align="center">
